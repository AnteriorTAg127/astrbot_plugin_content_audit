...
[![Version](https://img.shields.io/badge/version-v2.2.0-blue)](metadata.yaml)
...
- **上下文注入** — 缓存群聊最近 K 条消息，审核时作为上下文传入 API，辅助 LLM 做出更准确判断
...
- **列表排序与筛选** - 4 个列表支持点击表头排序（localStorage 持久化）；违规/审计按群下拉（含群名：别名->平台->群号）+ 时间区间过滤；用户档案按加群数排序 + 按群多选 + 首末出现时间区间查询
- **分群白名单** - 白名单支持全局（群号为空）与群级，消息检查命中全局或当前群即放行
- **多群管理命令** - 单绑定群自动作用，多绑定群需 `<群号|all>` 指定，未绑定群号拒绝
...
| **配置管理** | `config_manager.py` | 配置读写、热重载、智能审查决策、白名单查询、群级直读配置 |
...
【智能审查决策】
  ...
  └─ 应审查
       ...
       │
       ▼
  调用 POST /audit（附带上下文 context 字段）
       │
       ├─ 正常 → 放行
       │
       └─ 违规
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `audit.enabled` | bool | `true` | 总开关，关闭后所有群完全不审查 |
...
每个条目定义一个被管理群到管理群的绑定。**所有审核策略均在群级别直读**，不依赖全局回退：
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `skip_admin` | bool | `true` | 是否跳过管理员/群主的消息 |
| `skip_llm` | bool | `false` | 是否跳过 LLM 复核（仅用关键词+语义） |
| `min_text_length` | int | `2` | 最短审核文本长度 |
| `auto_recall` | bool | `true` | 是否自动撤回违规消息 |
| `auto_mute` | bool | `true` | 是否自动禁言违规用户 |
| `context_enabled` | bool | `true` | 是否将最近聊天记录作为上下文上传给审核 API |
| `context_max_messages` | int | `5` | 上下文最多携带的最近消息数 |
...
>
> 旧版 `override_*` 前缀配置项（如 `override_skip_admin`）仍兼容，但建议迁移到新版直读字段。
...
| `/文本审核 状态 [群号\|all]` | 查看审核统计；单绑定群自动，多绑定群需指定群号或 all | 无 |
| `/文本审核 日志 [群号\|all] [页码]` | 查看最近 10 条违规记录，可按群过滤 | 无 |
| `/文本审核 白名单 添加 <QQ号> [群号\|all]` | 添加白名单（all/缺省=全局，指定群号=群级） | 管理员 |
| `/文本审核 白名单 删除 <QQ号> [群号\|all]` | 从白名单移除用户 | 管理员 |
| `/文本审核 白名单 列表 [群号\|all]` | 列出白名单用户 | 无 |
| `/文本审核 删除违规 <QQ号> [群号\|all] 确认` | 清除指定用户的违规记录 | 管理员 |

---

## 上下文注入

启用 `context_enabled: true`（默认）后，插件会为每个群维护一个固定大小的消息缓冲区。审核时，插件将最近 K 条消息格式化为如下字符串，通过审核 API 的 `context` 字段传入：
...
上下文仅注入到 L3（LLM 审核）阶段的 Prompt 中，辅助大模型理解对话场景，减少误判。关键词匹配和语义检测阶段不受影响。

**设计要点**：当前消息本身不会出现在上下文中（避免重复）；消息内换行符会被转义为空格；上下文字符串上限 2000 字符；关闭上下文功能后完全不缓存，零开销。
